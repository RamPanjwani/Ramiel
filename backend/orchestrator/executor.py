...
class Executor:
    ...
    async def execute_task(self, prompt: str, session_id: str | None = None) -> TaskState:
    ...
    async def execute_plan(self, state: TaskState) -> TaskState:
        ...
        logger.info("executor.executing_plan", task_id=state.task_id, steps_count=len(state.plan))
        ...
        while state.current_step_index < len(state.plan):
            ...
            if requires_confirmation and not step.get("confirmed", False):
                logger.info("executor.confirmation_required", step=step_idx, action=step["description"])
                state.request_confirmation(
                    action=step["description"],
                    details={"step_index": step_idx, "tool_name": tool_name, "args": tool_args},
                )
                state.save_checkpoint()
                return state
            ...
            logger.info("executor.step_start", step=step_idx, model=model_id, tool=tool_name)
            ...
            if tool_name:
                ...
                last_output = state.observations[-1]["output"] if state.observations else ""
                ...
                if tool_name == "code_exec" and "code" not in resolved_args:
                    ...
                    resolved_args["content"] = str(last_output) or "Generated deliverable content."
                elif tool_name == "spreadsheet_write" and "data" not in resolved_args:
                    resolved_args["data"] = {"Calculations": ["Val1", "Val2"], "Values": [10.0, 20.0]}

                try:
                    tool_output = await self.tool_registry.execute(tool_name, **resolved_args)
                except (PermissionError, FileNotFoundError, TimeoutError, OSError, ValueError, KeyError, RuntimeError, TypeError) as exc:
                    logger.warning("executor.tool_failed", tool=tool_name, error=str(exc))
                    step_status = "failed"
                    ...
                    if state.retry_count < self.max_retries:
                        state.retry_count += 1
                        logger.info("executor.replanning_retry", retry=state.retry_count)
                        continue
                    else:
                        state.status = "failed"
                        state.add_observation(step_idx, tool_name, tool_output, status="failed")
                        state.save_checkpoint()
                        return state
            ...
            self.trace_store.record({
                ...
                "response": str(tool_output) if tool_output is not None else "Step completed",
            })
